<table width='100%'>
	<tr>
		<td align='left'>
		<h1> Invoice #{$invoice->billedinvoice_id} </h1><br>
		<h3> Client #{$invoice->customer_username}</h3><br>		
		{$invoice->customer_lastname} {$invoice->customer_firstname}<br>
		{$invoice->customer_address}<br>
		{$invoice->customer_zipcode} {$invoice->customer_city}<br>
		{$invoice->customer_state} {$invoice->customer_country}<br>
		Phone: {$invoice->customer_phone}<br>
		Email: {$invoice->customer_email}<br>
		Fax: {$invoice->customer_fax}<br>
		</td>
		<td align='right'>
		LOGO
		</td>		
