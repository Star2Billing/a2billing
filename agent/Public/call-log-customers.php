...
$FG_TABLE_NAME = "cc_call t1 LEFT OUTER JOIN cc_card t2 ON  t2.id = t1.card_id  LEFT OUTER JOIN cc_trunk t3 ON t1.id_trunk = t3.id_trunk LEFT OUTER JOIN cc_ratecard t4 ON t1.id_ratecard = t4.id LEFT JOIN cc_agent_cardgroup ON t2.id_group=cc_agent_cardgroup.id_card_group";
...
if (DB_TYPE != "postgres") {
	...
	$QUERY = "SELECT t1.starttime::date AS day, sum(t1.sessiontime) AS calltime, sum(t1.sessionbill) AS cost, count(*) as nbcall,
            sum(t1.buycost) AS buy, sum(case when t1.sessiontime>0 then 1 else 0 end) as success_calls
        	FROM $FG_TABLE_NAME WHERE " . $FG_TABLE_CLAUSE . " GROUP BY t1.starttime::date ORDER BY day"; //extract(DAY from calldate)
}
