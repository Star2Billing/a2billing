...
class Callback {
	...
	function Callback()
	{
        ...
        $this->__dispatch_map['Initiate_Conference'] = array (
			'in' => array ( 'security_key' => 'string', 'phonenumber_moderator' => 'string', 'phonenumber_member' => 'string', 'callerid' => 'string', 'callback_time' => 'string', 'uniqueid' => 'string', 'accountnumber' => 'string' ),
			'out' => array ( 'id' => 'string', 'result' => 'string', 'details' => 'string' )
		);
	...
	/*
	 *		Function to Initiate Conference : it will insert a callback request
	 */
	function Initiate_Conference($security_key, $called, $calling, $callerid, $callback_time, $uniqueid, $accountnumber)
	{
		// $called : PHONE NUMBER MODERATOR
		// $calling : PHONE NUMBER MEMBERS
		...
		// The wrapper variables for security
		// $security_key = API_SECURITY_KEY;
		write_log(LOG_CALLBACK, " Service_Initiate_Conference( security_key=$security_key, called=$called, calling=$calling, callerid=$callerid, uniqueid=$uniqueid, callback_time=$callback_time)");
		$mysecurity_key = API_SECURITY_KEY;
		...
		if (strlen($phone_number) < 9) {
			write_log(LOG_CALLBACK, basename(__FILE__) . ' line:' . __LINE__ . "[" . date("Y/m/d G:i:s", mktime()) . "] " . " ERROR FORMAT PHONENUMBER AT LEAST 10 DIGITS ");
			return array ( $insert_id_callback, 'result=Error', " ERROR - FORMAT PHONENUMBER AT LEAST 9 DIGITS " );
		}
		
		// CHECK DESTINATION NUMBER
		if (strlen($calling) < 9) {
			write_log(LOG_CALLBACK, basename(__FILE__) . ' line:' . __LINE__ . "[" . date("Y/m/d G:i:s", mktime()) . "] " . " ERROR FORMAT DESTINATION NUMBER AT LEAST 2 DIGITS ");
			return array ( $insert_id_callback, 'result=Error', " ERROR - FORMAT DESTINATION NUMBER AT LEAST 9 DIGITS " );
		}
        ...
        // CHECK ACCOUNT
		if ($A2B->credit <= 0) {
			write_log(LOG_CALLBACK, basename(__FILE__) . ' line:' . __LINE__ . "[" . date("Y/m/d G:i:s", mktime()) . "] " . " Accountnumber not found or not enough credit");
			return array ( $insert_id_callback, 'result=Error', " ERROR - Accountnumber not found or not enough credit" );
		}
		...
		if ($resfindrate != 0) {
			...
			if ($res_all_calcultimeout) {
				...
				$channel = $dialstr;
				$exten = $called;
				$context = 'a2billing-conference-moderator';
				$id_server_group = $A2B->config["callback"]['id_server_group'];
				$priority = 1;
				$timeout = $A2B->config["callback"]['timeout'] * 1000;
				$application = '';
				...
				$variable = "CALLED=$called".$sep."CALLING=$calling".$sep."CBID=$uniqueid".$sep."TARIFF=".$A2B->tariff.$sep.
                        "LEG=$accountnumber".$sep."ACCOUNTCODE=$accountnumber".$sep."PN_MEMBER=$calling".$sep."ROOMNUMBER=".$uniqueid;
