...
$QUERY_GRAPH_REFILL_COUNT = "SELECT UNIX_TIMESTAMP( DATE_FORMAT( date, '%Y-%m-01' ) ) , count( * )  FROM cc_logrefill WHERE date >= TIMESTAMP( '$checkdate' ) AND date <=CURRENT_TIMESTAMP GROUP BY MONTH( date ),date ORDER BY date;";
$result_graph_refill_count = $table -> SQLExec($DBHandle,$QUERY_GRAPH_REFILL_COUNT);
$QUERY_GRAPH_REFILL_AMOUNT = "SELECT UNIX_TIMESTAMP( DATE_FORMAT( date, '%Y-%m-01' ) ) , SUM( credit )  FROM cc_logrefill WHERE date >= TIMESTAMP( '$checkdate' ) AND date <=CURRENT_TIMESTAMP GROUP BY MONTH(date),date ORDER BY date;";
