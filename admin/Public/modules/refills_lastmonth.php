...
$QUERY_GRAPH_REFILL_COUNT = "SELECT UNIX_TIMESTAMP( DATE_FORMAT( date, '%Y-%m-01' ) ) , count( * ) , DATE_FORMAT( date, '%M' ) FROM cc_logrefill WHERE date >= TIMESTAMP( '$checkdate' ) AND date <=CURRENT_TIMESTAMP GROUP BY MONTH( date ) ORDER BY date;";
$result_graph_refill_count = $table -> SQLExec($DBHandle,$QUERY_GRAPH_REFILL_COUNT);
$QUERY_GRAPH_REFILL_AMOUNT = "SELECT UNIX_TIMESTAMP( DATE_FORMAT( date, '%Y-%m-01' ) ) , SUM( credit ) , DATE_FORMAT( date, '%M' ) FROM cc_logrefill WHERE date >= TIMESTAMP( '2008-01-01' ) AND date <=CURRENT_TIMESTAMP GROUP BY MONTH(date) ORDER BY date;";
